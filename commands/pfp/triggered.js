...
module.exports = class {
    ...
    async onChat(msg, args, config) {
        ...
        .then(async image => { return await image.resize( 600, Jimp.AUTO);  });
        tagged = tagged.posterize( 30 );

        var overlay = await Jimp.read('./assets/images/avatar/triggered.png')
        .then(async image => { return await image.resize(Jimp.AUTO, 600);  });

        var mentioned   = await Jimp.read('./assets/images/avatar/triggered.png')
        .then(async image => {
            return await image.resize(Jimp.AUTO, 600)
            .composite( tagged, 0, 0 )
            .composite( overlay, 0, 0 );
        });


        var x = await mentioned.getBufferAsync(Jimp.MIME_PNG);
        const attachment = new Discord.MessageAttachment(x, 'triggered.png');
