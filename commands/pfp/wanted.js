...
module.exports = class {
    ...
    async onChat(msg, args, config) {
        ...
        var overlay = await Jimp.read('./assets/images/avatar/wanted.png')
        .then(async image => { return await image.resize(Jimp.AUTO, 600);  });

        var mentioned   = await Jimp.read('./assets/images/avatar/wanted.png')
        .then(async image => {
            return await image.resize(Jimp.AUTO, 600)
            .composite( tagged, 80, 210 )
            .composite( overlay, 0, 0 );
        });


        var x = await mentioned.getBufferAsync(Jimp.MIME_PNG);
        const attachment = new Discord.MessageAttachment(x, 'wanted.png');
