...
module.exports = class {
    ...
    async onChat(msg, args, config) {
        ...
        .then(async image => { return await image.resize( 280, Jimp.AUTO);  });

        var overlay = await Jimp.read('./assets/images/avatar/new_foe.png')
        .then(async image => { return await image.resize(Jimp.AUTO, 600);  });

        var mentioned   = await Jimp.read('./assets/images/avatar/new_foe.png')
        .then(async image => {
            return await image.resize(Jimp.AUTO, 600)
            .composite( tagged, 700, 170 )
            .composite( overlay, 0, 0 );
        });


        var x = await mentioned.getBufferAsync(Jimp.MIME_PNG);
        const attachment = new Discord.MessageAttachment(x, 'new.png');
...
module.exports.config = {
    info: {
        ...
        trigger: 'new',
        aliases: [],
        tags: ['smash bros', 'super smash bros'],
        usage: '%trigger% <user>',
        color: 13244606
    },
