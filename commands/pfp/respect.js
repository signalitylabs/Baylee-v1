...
module.exports = class {
    ...
    async onChat(msg, args, config) {
        ...
        var overlay     = await Jimp.read(tagged)
        .then(async image => { return await image.resize(120, Jimp.AUTO);  });

        var mentioned   = await Jimp.read('./assets/images/avatar/image_respects.png')
        .then(async image => { return await image.resize(Jimp.AUTO, 600).composite( overlay, 170, 70 ); });


        var x = await mentioned.getBufferAsync(Jimp.MIME_PNG);
        const attachment = new Discord.MessageAttachment(x, 'respect.png');
