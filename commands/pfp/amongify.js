...
module.exports = class {
    ...
    async onChat(msg, args, config) {
        ...
        .then(async image => { return await image.resize(295, Jimp.AUTO);  });

        var overlay = await Jimp.read('./assets/images/avatar/amongify.png')
        .then(async image => { return await image.resize(Jimp.AUTO, 600);  });

        var mentioned   = await Jimp.read('./assets/images/avatar/amongify.png')
        .then(async image => {
            return await image.resize(Jimp.AUTO, 600)
            .composite( tagged, 51, 140 )
            .composite( overlay, 0, 0 );
        });


        var x = await mentioned.getBufferAsync(Jimp.MIME_PNG);
        const attachment = new Discord.MessageAttachment(x, 'amongify.png');
