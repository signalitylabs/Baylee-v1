...
module.exports = class {
    ...
    async onChat(msg, args, config) {
        ...
        .then(async image => { return await image.resize( 333, Jimp.AUTO);  });

        var overlay = await Jimp.read('./assets/images/avatar/bob_ross.png')
        .then(async image => { return await image.resize(Jimp.AUTO, 600);  });

        var mentioned   = await Jimp.read('./assets/images/avatar/bob_ross.png')
        .then(async image => {
            return await image.resize(Jimp.AUTO, 600)
            .composite( tagged, 16, 20 )
            .composite( overlay, 0, 0 );
        });


        var x = await mentioned.getBufferAsync(Jimp.MIME_PNG);
        const attachment = new Discord.MessageAttachment(x, 'paint.png');
